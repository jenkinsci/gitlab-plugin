...
class PushSystemHookTriggerHandlerImpl extends AbstractWebHookTriggerHandler<PushSystemHookEvent>
        implements PushSystemHookTriggerHandler {
    ...
    public PushSystemHookTriggerHandlerImpl(boolean triggerToBranchDeleteRequest) {
    ...
    @Override
    protected boolean isCiSkip(PushSystemHookEvent event) {
    ...
    @Override
    protected CauseData retrieveCauseData(PushSystemHookEvent event) {
        try {
    ...
    @Override
    protected String getSourceBranch(PushSystemHookEvent event) {
        return event.getRef() == null ? null : event.getRef().replaceFirst("^refs/heads/", "");
    }

    @Override
    protected String getTargetBranch(PushSystemHookEvent event) {
        return event.getRef() == null ? null : event.getRef().replaceFirst("^refs/heads/", "");
    }
    ...
    @Override
    protected RevisionParameterAction createRevisionParameter(PushSystemHookEvent event, GitSCM gitSCM)
    ...
    @Override
    protected BuildStatusUpdate retrieveBuildStatusUpdate(PushSystemHookEvent event) {
    ...
    @Override
    protected URIish retrieveUrIish(PushSystemHookEvent event) {
        try {
    ...
    private String retrievePushedBy(final PushSystemHookEvent event) {
    ...
    private String retrieveRevisionToBuild(PushSystemHookEvent event, GitSCM gitSCM) throws NoRevisionToBuildException {
    ...
    private boolean inNoBranchDelete(PushSystemHookEvent event) {
        return event.getAfter() != null && !event.getAfter().equals(NO_COMMIT);
    }

    private boolean isNoRemoveBranchPush(PushSystemHookEvent event) {
        return event.getAfter() != null && !event.getAfter().equals(NO_COMMIT);
    }
}
