...
- [Jenkins Gitlab Plugin](#jenkins-gitlab-plugin)
  ...
  - [Global plugin configuration](#global-plugin-configuration)
    - [GitLab-to-Jenkins authentication](#gitlab-to-jenkins-authentication)
    - [Configuring global authentication](#configuring-global-authentication)
    - [Configuring per-project authentication](#configuring-per-project-authentication)
    - [Disabling authentication](#disabling-authentication)
  - [Jenkins-to-GitLab authentication](#jenkins-to-gitlab-authentication)
...
1. Log in as that user (this is required even if you are a Jenkins admin user), then click on the user's name in the top right corner of the page
1. Click 'Configure,' then 'Show API Token...', and note/copy the User ID and API Token
1. In GitLab, when you create webhooks to trigger Jenkins jobs, use this format for the URL and do not enter anything for 'Secret Token': `http://USERID:APITOKEN@JENKINS_URL/project/YOUR_JOB`
...
1. In GitLab, create a webhook for your project, enter the trigger URL (e.g. `http://JENKINS_URL/project/YOUR_JOB`) and paste the token in the Secret Token field
1. After you add the webhook, click the 'Test' button, and it should succeed
...
1. Scroll down to the section labeled 'GitLab'
1. Uncheck "Enable authentication for '/project' end-point" - you will now be able to trigger Jenkins jobs from GitLab without needing authentication
...
1. Give this user 'Developer' permissions on each repo you want Jenkins to send build status to
1. Log in or 'Impersonate' that user in GitLab, click the user's icon/avatar and choose Settings
1. Click on 'Access Tokens'
1. Create a token named e.g. 'jenkins' with 'api' scope; expiration is optional
1. Copy the token immediately, it cannot be accessed after you leave this page
1. On the Global Configuration page in Jenkins, in the GitLab configuration section, supply the GitLab host URL, e.g. `http://your.gitlab.server`
1. Click the 'Add' button to add a credential, choose 'GitLab API token' as the kind of credential, and paste your GitLab user's API key into the 'API token' field
1. Click the 'Test Connection' button; it should succeed
...
1. Click *Git*
1. Enter your *Repository URL*, such as ``git@your.gitlab.server:gitlab_group/gitlab_project.git``
...
1. In *Branch Specifier* enter:
   ...
   1. For forked repository workflows: ``merge-requests/${gitlabMergeRequestIid}``
1. In *Additional Behaviours*:
   1. Click the *Add* drop-down button
   1. Select *Merge before build* from the drop-down
   1. Set *Name of repository* to ``origin``
   1. Set *Branch to merge* as ``${gitlabTargetBranch}``
...
1. Click **Add source**
1. Select **Git**
1. Enter your *Repository URL* (e.g.: ``git@your.gitlab.server:group/repo_name.git``)
...
1. Click *Save* to preserve your changes in Jenkins.
...
1. Your Declarative Pipeline jobs have more than one stage, and
1. You use a `gitlabCommitStatus` step *in each stage* to send status to GitLab...
...
1. In the *Flexible publish* section:
   1. *Add conditional action*
   1. In the *Conditional action* section:
      1. Set *Run?* to *Never*
      1. Select *Condition for Matrix Aggregation*
      1. Set *Run on Parent?* to *Always*
      1. Add GitLab actions as required
...
1. In the job configuration under 'Source code management':
   1. Select 'Advanced...' and add '+refs/tags/*:refs/remotes/origin/tags/*' as the Refspec
   1. You can also use 'Branch Specifier' to specify which tag need to be built (example 'refs/tags/${TAGNAME}')
